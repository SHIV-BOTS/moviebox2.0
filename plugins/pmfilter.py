...
async def qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn = []
    valid_quals = [x for x in QUALITIES if x.strip()]
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    valid_langs = [x for x in LANGUAGES if x.strip()]
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    valid_langs = [x for x in LANGUAGES if x.strip()]
    for x in valid_langs:
        if x.lower() in search.lower():
            search = re.sub(rf"(?i)\b{x}\b", "", search).strip()
    ...
    if lang != "homepage":
        if lang.lower() == "hindi":
            search = f"{search} hin".strip()
        elif lang.lower() == "english":
            search = f"{search} eng".strip()
        elif lang.lower() == "tamil":
            search = f"{search} tam".strip()
        elif lang.lower() == "telugu":
            search = f"{search} tel".strip()
        elif lang.lower() == "malayalam":
            search = f"{search} mal".strip()
        else:
    ...
    if offset != "":
        ...
        btn.append([InlineKeyboardButton(text="↭ ɴᴏ ᴍᴏʀᴇ ᴘᴀɢᴇꜱ ᴀᴠᴀɪʟᴀʙ ↭",callback_data="pages")])
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    valid_seas = [x for x in SEASONS if x.strip()]
    for i in range(0, len(valid_seas), 2):
        row = [InlineKeyboardButton(text=valid_seas[i].title(), callback_data=f"fs#{valid_seas[i].lower()}#{key}")]
        if i + 1 < len(valid_seas):
            row.append(InlineKeyboardButton(text=valid_seas[i+1].title(), callback_data=f"fs#{valid_seas[i+1].lower()}#{key}"))
        btn.append(row)
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    for x in season_search:
        if x in search.lower():
            search = re.sub(rf"(?i)\b{x}\b", "", search).strip()

    num = ''.join(filter(str.isdigit, seas))
    if num:
        s_code = f"S{num.zfill(2)}"
        search = f"{search} {s_code}".strip()
    else:
        search = f"{search} {seas}".strip()
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        files, n_offset, total = await get_search_results(query.message.chat.id, search, offset=int(offset), filter=True)
        await send_all(client, query.from_user.id, files, ident, query.message.chat.id, query.from_user.first_name, query)
        await query.answer(f"ʜᴇʏ {query.from_user.first_name}, ᴀʟʟ ꜰɪʟᴇꜱ ᴏɴ ᴛʜɪꜱ ᴘᴀɢᴇ ʜᴀꜱ ʙᴇᴇɴ ꜱᴇɴᴛ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟ ᴛᴏ ʏᴏᴜ ʙʏ ᴅᴍ !", show_alert=True)
