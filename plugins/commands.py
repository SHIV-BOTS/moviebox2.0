    ...
    if channels:  
        ...
        if btn:
            try:
                ...
                # Handle case where there's no underscore in the command
                kk = "getfile"
        ...
        for file in files:
            ...
            if not await check_verification(client, message.from_user.id) and VERIFY == True:
                ...
                l = await message.reply_text(
                    text="<b>♨️ YOU ARE NOT VERIFIED !\nKINDLY VERIFY TO CONTINUE & YOU CAN GET UNLIMITED ACCESS FOR 24 HOURS ✅\n\n<blockquote>⚜️ इस BOT से MOVIE प्राप्त करने के लिए आपको VERIFY करना आवश्यक है VERIFY करने के बाद आप 24 घंटे तक UNLIMITED MOVIES प्राप्त कर सकते हो।</blockquote></b>",
                    protect_content=False,
                    reply_markup=InlineKeyboardMarkup(btn)
                )
...
async def settings(client, message):
    ...
    for key, default_value in required_keys.items():
        if key not in settings:
            await save_group_settings(grp_id, key, default_value)
    
    # Refresh settings after adding missing keys
    settings = await get_settings(grp_id)
    ...
    if settings is not None:
        buttons = [        
            ...
            [
                InlineKeyboardButton(
                    '📡 FTM Gamma Mode',
                    callback_data=f'setgs#ftm_gamma_mode#{settings.get("ftm_gamma_mode", False)}#{grp_id}',
                ),
                InlineKeyboardButton(
                    '✅ Enabled' if settings.get("ftm_gamma_mode", False) else '❌ Disabled',
                    callback_data=f'setgs#ftm_gamma_mode#{settings.get("ftm_gamma_mode", False)}#{grp_id}',
                ),
            ],
            [
...
async def confirmation_handler(client, callback_query):
    ...
    if action == "yes":
        ...
        await callback_query.message.edit_text(
            '🧹 ᴜᴘᴅᴀᴛᴇꜱ ᴄʜᴀɴɴᴇʟ ʟɪsᴛ ʜᴀs ʙᴇᴇɴ ᴄʟᴇᴀʀᴇᴅ sᴜᴄssғᴜʟʟʏ ✅'
        )
    elif action == "no":
